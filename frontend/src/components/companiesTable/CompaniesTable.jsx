...
const CompaniesTable = () => {
    ...
    const { data, isLoading, isError, error, isSuccess } = useSelector((state) => state.getCompany);
    ...
    const handlePageChange = (event, newPage) => {
        ...
        dispatch(getCompanyTable({ search, gender, status, sort, page: newPage }))
    ...
    const handleSearch = (searchedVal) => {
        ...
        setCurrentPage(1);
        if (searchedVal == "") {
            dispatch(getCompanyTable({ search: searchedVal, gender, status, sort, page: 1 }))
        }
    }

    const requestSearch = () => {
        setCurrentPage(1);
        dispatch(getCompanyTable({ search, gender, status, sort, page: 1 }))
    };

    const handleGenderChange = (event) => {
        setGender(event.target.value);
        setCurrentPage(1);
        dispatch(getCompanyTable({ search, gender: event.target.value, status, sort, page: 1 }))
    };

    const handleStatusChange = (event) => {
        setStatus(event.target.value);
        setCurrentPage(1);
        dispatch(getCompanyTable({ search, gender, status: event.target.value, sort, page: 1 }))
    };

    const handleSortChange = (event) => {
        setSort(event.target.value);
        setCurrentPage(1);
        dispatch(getCompanyTable({ search, gender, status, sort: event.target.value, page: 1 }))
    };

    useEffect(() => {
        dispatch(getCompanyTable({ search, gender, status, sort, page: currentPage }))
